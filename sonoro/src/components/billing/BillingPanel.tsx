...
export default function BillingPanel({ overview, tiers: tiersProp, checkoutSuccess }: Props) {
  ...
  const tiers: TierConfig[] = Array.isArray(tiersProp)
    ? tiersProp
    : Array.isArray((tiersProp as any)?.tiers)
    ? (tiersProp as any).tiers
    : [];

  const currentTier = overview?.user?.plan_tier ?? 'FREE';
  const hasSubscription = Boolean(overview?.billing?.status && overview.billing.status !== 'free');
  ...
  const usage = overview?.usage ?? {
    chars_used: 0, chars_limit: 0,
    jobs_created: 0, jobs_limit: 0,
    storage_mb: 0, storage_limit_mb: 0,
  };

  const { chars_used, chars_limit, jobs_created, jobs_limit, storage_mb, storage_limit_mb } = usage;
    ...
    <div className="space-y-6">
      ...
      <div className="card-base p-6">
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            {overview?.billing?.current_period_end && (
              <p className="text-xs text-sonoro-muted mt-1">
                Renews {new Date(overview.billing.current_period_end).toLocaleDateString('en-US', {
          ...
          {/* Only show "Manage billing" when user has a Stripe subscription */}
      ...
      {currentTier !== 'ENTERPRISE' && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-semibold text-sonoro-900">
              {currentTier === 'FREE' ? 'Choose a plan' : 'Available plans'}
            </h2>
          ...
          {tiers.length === 0 ? (
            <div className="rounded-xl border border-sonoro-border bg-sonoro-surface px-6 py-8 text-center">
              <p className="text-sm text-sonoro-muted">Plan information unavailable — try refreshing.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {tiers.map((tier) => {
                const isCurrent   = tier.tier === currentTier;
                const isHigher    = (TIER_RANK[tier.tier] ?? 0) > (TIER_RANK[currentTier] ?? 0);
