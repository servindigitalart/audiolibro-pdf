 ...
 *
 * This is a navigation UX: it shows what was playing and links back
 * to the document page with ?autoplay=1 to resume. It does not control
...
export default function MiniPlayer() {
  ...
  useEffect(() => {
    ...
    // Only show when meaningfully into a book (>30s) and not at the very end (>5% remaining)
    if (p.currentTime < 30 || pct >= 0.95) return;

    // Hide on the document page that's already playing this book
    const isOnDocumentPage = window.location.pathname === `/dashboard/documents/${p.documentId}`;
    if (isOnDocumentPage) return;

    setProgress(p);
    // Small delay so the player slides up after the page has settled
    ...
    >
      ...
      <div className="bg-sonoro-white border-t border-sonoro-border shadow-modal">
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 max-w-screen-xl mx-auto">
          ...
          <a
            href={resumeHref}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-sonoro-black text-white hover:bg-sonoro-800 active:scale-95 transition-all shadow-card"
            aria-label="Resume playback"
          >
            <svg className="w-4 h-4 translate-x-px" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
