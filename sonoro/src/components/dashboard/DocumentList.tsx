...
export default function DocumentList({ initialDocuments }: Props) {
  ...
  async function handleRetry(id: string, stuck: boolean) {
    ...
    try {
      ...
      // Navigate to the full processing UI so the user sees detailed progress.
      window.location.href = `/dashboard/upload?retryDocumentId=${id}`;
    } catch (err) {
      setRetrying(null);
      alert(getErrorMessage(err));
    }
    // Do not call setRetrying(null) on success — we are navigating away.
  }
